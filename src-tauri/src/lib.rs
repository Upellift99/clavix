...
#[tauri::command]
async fn start_ssh_agent(state: State<'_, AppState>) -> Result<SshAgentStatus> {
    ...
    let decrypted: Vec<(String, String, String)> = {
        ...
        vault
            ...
            .iter()
            .filter(|c| c.deleted_date.is_none())
            .filter(|c| matches!(c.kind, models::CipherType::SshKey))
            .filter_map(|c| {
...
#[tauri::command]
async fn stop_ssh_agent(state: State<'_, AppState>) -> Result<()> {
    let handle = {
        let mut slot = state.ssh_agent.lock().unwrap();
        slot.take()
    };
...
#[tauri::command]
fn ssh_agent_status(state: State<'_, AppState>) -> Result<SshAgentStatus> {

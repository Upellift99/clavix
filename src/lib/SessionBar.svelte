<script lang="ts">
  ...
  // No token-shaped prop on purpose: the access/refresh tokens and
  // expires_in are deliberately kept Rust-side (see
  // `src-tauri/src/models.rs::TokenSet` — "intentionally token-free:
  // … never cross the IPC boundary"). The bar's job here is the
  // sync-health signal, not session metadata. If session expiry has
  // to surface in the UI later, add a dedicated safe IPC that
  // exposes only `expires_at` (or a fingerprint) — never the token.
  type Props = {
    syncing: boolean;
    hasSync: boolean;
